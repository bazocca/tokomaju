...
?>
	<fieldset>
		<script>
			$(document).ready(function(){
                if($('#form-title-entry').length > 0)
                {
                    $('#form-title-entry').html('edit stock / ketr barang gudang');
                    $('input#barang-dagang').nextAll('a.get-from-table').addClass('hide');
                    $('input#barang-dagang').nextAll('p.help-block').addClass('hide');
                }
