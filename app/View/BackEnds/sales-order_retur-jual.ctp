...
<script>
	$(document).ready(function(){
		...
		<?php if(empty($popup)): ?>
			...
			// UPDATE TITLE HEADER !!
			$('div.title > h2').html('RETUR PENJUALAN');
			...
			if($mySetting['table_view'] == "complex")
			{
				...
				foreach ( $metaFields['TypeMeta'] as $key => $value) 
				{
					if(substr($value['TypeMeta']['key'], 0,5) == 'form-')
					{
                        ...
                        if(!empty($popup) && $this->request->query['key'] == $shortkey || $shortkey == 'status_bayar' || $shortkey == 'diskon_nota' || $shortkey == 'ongkos_tambahan' || $shortkey == 'laba_bersih' || $shortkey == 'uang_muka')
                        {
                            $hideKeyQuery = 'hide';
                        }
